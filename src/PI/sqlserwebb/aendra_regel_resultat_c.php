...
function rule_post_value($key)
{
    return isset($_POST[$key]) ? trim((string) $_POST[$key]) : "";
}

function normalize_select_value($value, $placeholder)
{
...
$a_regel_c_id = isset($_SESSION['a_regel_c_id']) ? (string) $_SESSION['a_regel_c_id'] : "";
...
if (!ctype_digit($regel_id) || (int) $regel_id <= 0) {
    $alerts[] = 'Ogiltigt regel-id!';
} elseif ($land_till == 'Ange land') {
    $alerts[] = 'Land måste anges som sökfält!';
} elseif (strlen($org_till) == 0) {
    $alerts[] = 'Organisation måste anges som sökfält!';
} elseif ($org_1_till == 'Ange organisation' || strlen($org_1_till) == 0) {
    $alerts[] = 'Organisation 1 måste anges som ändringsfält!';
} elseif ($delas_till == 1) {
    if (($org_2_till != 'Ange organisation' || $org_3_till != 'Ange organisation') && (strlen($org_2_till) > 0 || strlen($org_3_till) > 0 )) {
        $alerts[] = 'Antalet i Delas stämmer inte med antal angivna organisationer!';
    } else {
        $koll_svar = true;
    }
} elseif ($delas_till == 2) {
    ...
    $alerts[] = 'Antalet i Delas kan vara mellan 1 och 3!';
}
...
if ($koll_svar && $a_regel_c_id !== $regel_id) {
    try {
        ...
        $sql = "UPDATE Rule_center_match SET
        ...
        if ($stmt->rowCount() > 0) {
            $alerts[] = 'Regeln är nu ändrad!';
            $_SESSION['a_regel_c_id'] = $regel_id;
            $updated = true;
        } else {
            $alerts[] = 'Fel vid ändring av regeln!';
        }
...
<script>alert("<?php echo bibmet_h($alert); ?>");</script>
<?php endforeach; ?>
...
<form action="aendra_regel_resultat_c.php" method="post">
    ...
    <h3>SÖKFÄLT</h3>
    Land:</br><input type="text" name="Land" value="<?php echo bibmet_h($land_till); ?>" disabled />
    <br />
    Stad:</br><input type="text" name="Stad" value="<?php echo bibmet_h($stad_till); ?>" disabled />
    <br />
    Organisationsnamn:</br><input type="text" name="Organisation" value="<?php echo bibmet_h($org_till); ?>" disabled /><br />

    <h3>ÄNDRINGSFÄLT</h3>
    Delas i:</br><input type="text" name="Delas" size="1" value="<?php echo bibmet_h($delas_till); ?>" disabled /><br /><br />

    <b>Organisation 1:</b><br />
    Annat organisationsnamn:<br /><input type="text" size="20" value="<?php echo bibmet_h($org_1_till); ?>" disabled />
    <br />Annat land:<br /><input type="text" size="20" value="<?php echo bibmet_h($land_1_till); ?>" disabled />
    <br />Annan stad:<br /><input type="text" value="<?php echo bibmet_h($stad_1_till); ?>" disabled /><br /><br />

    <b>Organisation 2:</b><br />
    Annat organisationsnamn:<br /><input type="text" size="20" value="<?php echo bibmet_h($org_2_till); ?>" disabled />
    <br />Annat land:<br /><input type="text" value="<?php echo bibmet_h($land_2_till); ?>" disabled />
    <br />Annan stad:<br /><input type="text" value="<?php echo bibmet_h($stad_2_till); ?>" disabled /><br /><br />

    <b>Organisation 3:</b><br />
    Annat organisationsnamn:<br /><input type="text" size="20" value="<?php echo bibmet_h($org_3_till); ?>" disabled />
    <br />Annat land:<br /><input type="text" size="20" value="<?php echo bibmet_h($land_3_till); ?>" disabled />
    <br />Annan stad:<br /><input type="text" value="<?php echo bibmet_h($stad_3_till); ?>" disabled /><br /><br />
</form>
</body>
</html>
